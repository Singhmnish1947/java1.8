 ...
 *
 * ********************************************************************************
 * $Id: UB_SWT_PopulateMT320.java,v.1.0,Sep 24, 2008 5:16:17 PM Sarun.Selvanesan
 *
 */
...
import com.trapedza.bankfusion.steps.refimpl.AbstractUB_SWT_PopulateMT320;
import com.trapedza.bankfusion.steps.refimpl.IUB_SWT_PopulateMT320;
...
public class UB_SWT_PopulateMT320 extends AbstractUB_SWT_PopulateMT320 implements IUB_SWT_PopulateMT320 {
    ...
    public UB_SWT_PopulateMT320(BankFusionEnvironment env) {
        super(env);
        // TODO Auto-generated constructor stub
    }

    private transient final static Log logger = LogFactory.getLog(UB_SWT_PopulateMT320.class.getName());
     ...
     * (non-Javadoc)
     * 
     * @see com.trapedza.bankfusion.steps.refimpl.AbstractSWT_MT320Validate#process(com.trapedza.
    ...
    public void process(BankFusionEnvironment env) {
        ...
        if (disposalObject != null) {
            logger.debug("INSIDE 320 " + disposalObject.getDisposalRef());
            ...
            boolean generate320 = util.generateCategory2Message(((UB_SWT_DisposalObject) disposalObject).getValueDate(),
                    ((UB_SWT_DisposalObject) disposalObject).getPostDate(), env,
                    ((UB_SWT_DisposalObject) disposalObject).getContraAccCurrencyCode(),
                    new java.sql.Date(bankFusionSystemDate.getTime()), "320");
            ...
            if (generate320) {
                ...
                try {
                    ...
                    contraAccDescription = SWTPostingUtils.getCustomerDetails(accountBO.getF_CUSTOMERCODE(),
                            disposalObject.getContraAccountNo());
                }
                catch (Exception e) {
                ...
                try {
                    ...
                    /*
                     * mainAccCustDetails = (IBOSwtCustomerDetail)
                     * env.getFactory().findByPrimaryKey( IBOSwtCustomerDetail.BONAME,
                     * disposalObject.getMainAccCustomerNumber()); // contra account customer
                     * details contraAccCustDetails = (IBOSwtCustomerDetail)
                     * env.getFactory().findByPrimaryKey( IBOSwtCustomerDetail.BONAME,
                ...
                if (disposalObject.getCancelFlag() == 0 && (disposalObject.getRelatedDealNumber() != null
                ...
                else messageObject_320.setSendersReference(disposalObject.getCurrentDealNumber());
                ...
                if (disposalObject.getTransactionStatus().equals("BRKDEP") || disposalObject.getTransactionStatus().equals("BRKLON")
                        || disposalObject.getTransactionStatus().startsWith("BR")) {
                    messageObject_320.setTypeOfOperation("CANC");
                }

                if ((disposalObject.getTransactionStatus().equals("AMEND")
                        || disposalObject.getTransactionStatus().startsWith("AM")) && disposalObject.getCancelFlag() != 0) {
                ...
                messageObject_320.setTermsAndConditions(util.getTermsAndConditionForDeals(disposalObject));
                // 39M fields PaymentClearingCentre
                messageObject_320.setPaymentClearingCentre(
                        !StringUtils.isBlank(disposalObject.getPaymentClearingCentre()) ? disposalObject.getPaymentClearingCentre()
                                : StringUtils.EMPTY);
                ...
                String str14D = null;
                if (disposalObject.getDayCountFraction().length() <= 0)
                    str14D = get14DTagString(disposalObject.getMainAccCurrencyCode(), env);
                else str14D = "ACT/" + disposalObject.getDayCountFraction();
                if (str14D != null)
                    messageObject_320.setDayCountFraction(str14D);
                else {
                    /*
                     * throw new BankFusionException(127, new Object[] {
                     * "YearDays not found in currency table" }, logger, env);
                     */
                    EventsHelper.handleEvent(CommonsEventCodes.E_AN_UNEXPECTED_ERROR_OCCURRED,
                            new Object[] { "YearDays not found in currency table" }, new HashMap(), env);
                ...
                if (interDetailsExistFlag) {
                    String tempString = util.createSwiftTagString(payToBICCode, disposalObject.getSI_PayToPartyIdentifier(),
                    ...
                    if (!tempString.equals(EMPTYSTRING)) {
                        ...
                        String tempString1 = util.createSwiftTagString(intermediaryBICCode,
                    ...
                    String tempString = util.createSwiftTagString(payToBICCode, disposalObject.getSI_PayToPartyIdentifier(),
                            disposalObject.getSI_PayToText1(), disposalObject.getSI_PayToText2(), disposalObject.getSI_PayToText3(),
                            disposalObject.getSI_PayToText4());
                    if (!tempString.trim().equals(EMPTYSTRING)) {
                ...
                if (contraAccIsNostroAcc && contraAccCustDetails != null) {
                    ...
                    else receiveagent571 = contraAccCustDetails.getF_BICCODE().trim();
                    tag571 = "A";
    ...
    private HashMap getDates(String prevDisposal, BankFusionEnvironment env) {
        HashMap dateMap = new HashMap();
        IBOSWTDisposal Disposal = (IBOSWTDisposal) env.getFactory().findByPrimaryKey(IBOSWTDisposal.BONAME, prevDisposal);
    ...
    private String value32B(BankFusionEnvironment env) {
        ...
        String value32B = util.DecimalRounding(disposalObject.getContractAmount().abs().toString(),
    ...
    private String value32H(BankFusionEnvironment env) {
        ...
        if (!(codeWord.toUpperCase().indexOf("NEW") != -1)) {
            ...
            if (!(codeWord.toUpperCase().indexOf("CANCEL") != -1)) {

                value32H = util.DecimalRounding(disposalObject.getTransactionAmount().abs().toString(),
                        util.noDecimalPlaces(disposalObject.getMainAccCurrencyCode(), env));
                ...
                if ((codeWord.toUpperCase().indexOf("DEINC") != -1 || codeWord.toUpperCase().indexOf("LODEC") != -1
                        ...
                        && (disposalObject.getTransactionAmount().abs().compareTo(CommonConstants.BIGDECIMAL_ZERO) != 0)) {
    ...
    private String value34E(BankFusionEnvironment env) {
        ...
        String value34E = util.DecimalRounding(disposalObject.getInterestAmount().abs().toString(),
                util.noDecimalPlaces(disposalObject.getMainAccCurrencyCode(), env));
        value34E = disposalObject.getMainAccCurrencyCode() + value34E;
        if ((disposalObject.getPayReceiveFlag().compareTo("R") == 0)
                && disposalObject.getInterestAmount().compareTo(BigDecimal.ZERO) != 0) {
