...
import com.trapedza.bankfusion.steps.refimpl.AbstractSWT_ValidateDetails;
import com.trapedza.bankfusion.steps.refimpl.ISWT_ValidateDetails;
...
public class SWT_ValidateDetails extends AbstractSWT_ValidateDetails implements ISWT_ValidateDetails {
    ...
    public SWT_ValidateDetails(BankFusionEnvironment env) {
        super(env);
    }

    private transient final static Log logger = LogFactory.getLog(SWT_ValidateDetails.class.getName());
    int payment_Flag = 9;
    int receipt_Flag = 9;
    int confirm_Flag = 9;
    int cancel_Flag = 9;
    ...
    private static final String fetchCustomer = "SELECT T1.boID AS CUSTOMERCODE, T1.f_ADVICEDAYS  AS ADVICEDAYS, T1.f_CRCONFIRMRE"
            + "QUIRED AS CRCONFIRMREQUIRED,T1.f_DRCONFIRMREQUIRED AS DRCONFIRMREQUIRED, T1.f_FX"
            + "DEALCONFIRMREQUIRED AS FXDEALCONFIRMREQUIRED,T1.f_DEPOSITLOANCONFIRMREQUIRED AS "
            + "DEPOSITLOANCONFIRMREQUIRED, T1.f_ISFINANCIALINSTITUTE AS ISFINANCIALINSTITUTE,T1"
            + ".f_PAYMSGREQUIRED AS PAYMSGREQUIRED, T1.f_COVERMSGFLAG AS COVERMSGFLAG, T1.f_ADV"
            + "ICEMSGREQUIRED AS ADVICEMESSAGE,T1.f_BICCODE AS BICCODE, T1.f_SWTACTIVE as SWTACTIVE FROM "
            + IBOSwtCustomerDetail.BONAME + " T1 , " + IBOAccount.BONAME + " T2 WHERE T1.boID = T2.f_CUSTOMERCODE "
            + "AND T2.boID = ?";

    private static final String fetchDisposal = "SELECT T1.boID AS SWTDISPOSALID, T1.f_CUSTACCOUNTID as CUSTACCOUNTID,T1.f_CONTRAACCOUNTID as CONTRAACCOUNTID,T1.f_PAYMENTFLAG as PAYMENTFLAG ,"
            ...
            + "SELECT T2.f_PREVSWTDISPOSALID FROM " + IBOSWTDisposal.BONAME + " T2 WHERE T2.f_DEALNO = ?) and T1.f_DEALNO = ?";
    private static final String fetchDisposal1 = "SELECT T1.boID AS SWTDISPOSALID, T1.f_CUSTACCOUNTID as CUSTACCOUNTID,T1.f_CONTRAACCOUNTID as CONTRAACCOUNTID,T1.f_MESSAGESTATUS AS MESSAGESTATUS,T1.f_PAYMENTFLAG as PAYMENTFLAG ,"
     ...
     * (non-Javadoc)
     * 
     * @see com.trapedza.bankfusion.steps.refimpl.AbstractSWT_ValidateDetails#process(com.trapedza.
    ...
    public void process(BankFusionEnvironment env) {
        ...
        try {
            ...
            setF_IN_MainAccountCurrencyCode(mainAccount.getF_ISOCURRENCYCODE());
        ...
        customerConfigDetail = env.getFactory().executeGenericQuery(fetchCustomer, params, null);
        if (customerConfigDetail.size() > 0) {
            ...
            if (getF_IN_FXTransaction().intValue() == 0)
                setF_IN_Customer_Number((String) mainCustConfig.getDataMap().get(CUSTOMER_CODE));
        }
        else {
                logger.error(CUSTOMER_NOT_AVAILABLE_FOR_ACCOUNT + getF_IN_Main_Account());
        ...
        if ((getF_IN_FXTransaction().intValue() == 1) || (getF_IN_FXTransaction().intValue() == 2)) {
            try {
                fxCustomer = (IBOSwtCustomerDetail) factory.findByPrimaryKey(IBOSwtCustomerDetail.BONAME, getF_IN_Customer_Number(),
                        false);
                setF_IN_Customer_Number(fxCustomer.getBoID());
        ...
        if (getF_IN_Message_Type().equals(NUM_900) || getF_IN_Message_Type().equals(NUM_910)) {
            setF_IN_PrevDisposalID(getF_IN_DisposalID());
        }
        ...
        if (getF_IN_Code_Word().trim().indexOf(ROL) != -1 || getF_IN_Code_Word().trim().indexOf(MAT) != -1
            ...
            if (!disposalRecord.isEmpty()) {
                ...
                setF_IN_PrevDisposalID(prevDisposalID);
            }
        }
        ...
        if (getF_IN_Message_Type().equals(NUM_350)
            ...
            if (DealType.equalsIgnoreCase(NEW)) {
                ...
                setF_IN_PrevDisposalID(getF_IN_DisposalID().trim());
                try {
                    params.clear();
        ...
        if (isSwiftCurrency(SwtMainCurrency, SwtContraCurrency)) {
            setF_IN_Confirm_Flag(new Integer(confirm_Flag));
            setF_IN_Cancel_Flag(new Integer(cancel_Flag));
            setF_IN_Error_Flag(new Integer(error_Flag));
            setF_IN_Payment_Flag(new Integer(payment_Flag));
            setF_IN_Receipt_Flag(new Integer(receipt_Flag));
            setF_IN_prevCancelFlag(new Integer(prevCancelFlag));
            setF_IN_DealType(DealType);
            setF_IN_CancelMessage(new Boolean(cancelMessage));
            setF_IN_DRCRConfirmation(new Integer(CRDRconfirm_Flag));
            setF_IN_MainAccountCurrencyCode(currency1);
            Status_flag = NUM_0;
        }
        setF_OUT_prevMsgStatus(new Integer(0));

        if (Status_flag.compareTo("0") == 0) {
    ...
    /**
     * This method will compare the currencies
     * 
     * @param cur1
     * @param cur2
     * @return
    ...
    private void NewDecideMessage() {
        ...
        if (getF_IN_DRCRFlag().compareTo(NUM_0) == 0 || getF_IN_DRCRFlag().compareTo(NUM_1) == 0
                || getF_IN_DRCRFlag().compareTo(NUM_2) == 0)
            Other_Message_required();
    }
    ...
    private void MT320_MT330_Message_required() {
        ...
        if (contraCustomerSwiftActive.compareTo(YES) == 0) {
            if (isContraAccountIsNostro) {
                ...
                if (coverFlag == 0 || coverFlag == 2) {
                    if (((codeWord.indexOf(MATDEP) != -1 && !getF_IN_IntPayCap().trim().equalsIgnoreCase(C))
                            || codeWord.indexOf(BRKDEP) != -1 && !(getF_IN_DealOriginator().equals(NOMT202)))
        ...
        if (getF_IN_Code_Word().trim().equalsIgnoreCase(PAYCAP))
            setF_IN_Code_Word(NEW);
        setF_IN_MT350Required(MT350MessageRequired());
    }
    ...
    /**
     * This method checks whether to generate other message
     */
    private void Other_Message_required() {
        ...
        {
            CRDRconfirm_Flag = 0;
            if (getF_IN_DealOriginator().equalsIgnoreCase(F))
                payment_Flag = 9;
        }
        if (((getF_IN_Message_Type().compareTo(MT910) == 0 || getF_IN_DRCRFlag().compareTo("1") == 0)
                && !(codeWord.indexOf(ROLLOSAM) != -1) // the transaction is not Roll over loan with
                // same details
                && !(codeWord.indexOf(ROLDESAM) != -1) // the transaction is not Roll over deposit
                // with same details
