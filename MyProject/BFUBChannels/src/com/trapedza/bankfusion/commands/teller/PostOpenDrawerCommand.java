 ...
 * ********************************************************************************
 *
 * $Id: PostOpenDrawerCommand.java,v 1.6 2008/08/12 20:15:27 vivekr Exp $
 *
 * CVS/RCS Ident Block inserted at Wed Feb 11 12:31:38 GMT 2004
 * ------------------------------------------------------------
 *
 * $Log: PostOpenDrawerCommand.java,v $
 * Revision 1.6  2008/08/12 20:15:27  vivekr
 ...
 *
 * Revision 1.6  2004/02/17 09:55:01  angus
 * Automatic: add revision trace
 *
 * Revision 1.5  2004/02/11 12:31:39  angus
 * Automatic: add CVS/RCS Ident Block & revision constant
 *
 * ------------------------------------------------------------------------------------
 * File name	: PostOpenDrawerCommand.java
 * Author 	: vaishalih
 ...
 * @author vaishalih
 * 
 *         Used only for Teller. This command is fired by BFTC when OpenDrawer BP ends its
 *         execution. This is expected to update UserSessionManager's DrawerNumber from 'SUSPENSE'
 *         to logged in DrawerNumber.
 */
public class PostOpenDrawerCommand extends AbstractCommand {
    ...
    public Object execute() {
        ...
        params.add(usm.getUserId());
        params.add("Y");

        String whereClause = "where " + IBOCashDrawer.CURRENTUSERID + " = ? AND " + IBOCashDrawer.INUSE + " = ?";

        Iterator sourceIt = environment.getFactory().findByQuery(IBOCashDrawer.BONAME, whereClause, params, null).iterator();

        if (sourceIt.hasNext()) {

            String drwNo = ((IBOCashDrawer) sourceIt.next()).getF_DRAWERNUMBER();
            if (!(drwNo.equals("SUSPENSE"))) {
                usm.setDrawerNumber(drwNo);
                return Boolean.TRUE;
            }
            // else
            String eMessage = BankFusionMessages.getInstance().getFormattedEventMessage(40000135, new Object[] {},
                    BankFusionThreadLocal.getUserSession().getUserLocale());

            throw new BankFusionException(40000135, eMessage);
        }
        return null;
    }
    ...
    /**
     * Sets <code>null</code> as this command is internal and will always be transformed by the
     * standard <code>CmdHlprMessageTransformer</code>. It doesn't have a corresponding
