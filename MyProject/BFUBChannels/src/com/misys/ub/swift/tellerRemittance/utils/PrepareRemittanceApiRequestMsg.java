...
import com.finastra.api.paymentInititation.PreCalculatedCharges;
 ...
 *
 */
public class PrepareRemittanceApiRequestMsg {
	private transient final static Log LOGGER = LogFactory.getLog(PrepareRemittanceApiRequestMsg.class);

	public PaymentInitiationRequest preparePaymentInitiationRequest(TellerRemittanceRq remittanceRq) {
		PaymentInitiationRequest payInitRq = new PaymentInitiationRequest();
		// 1st part
	...
	private InitiationContext prepareInitiationContext(TellerRemittanceRq remittanceRq) {
		...
		if (remittanceRq.getTxnfeesInformation() != null) {
			...
			isChargeWaived = txnfeesInformation.getIsWaived();
		}

		// logic for pre-calculated charges
		if (isChargeWaived && null != remittanceRq.getTxnAdditionalDtls().getConsolidatedChargeAmount().getAmount()
				&& remittanceRq.getTxnAdditionalDtls().getConsolidatedChargeAmount().getAmount()
						.compareTo(BigDecimal.ZERO) == 0) {
			PreCalculatedCharges preCalculatedCharges = new PreCalculatedCharges();
			preCalculatedCharges.setAmount(BigDecimal.ZERO);
			preCalculatedCharges.setCurrency(
					null != remittanceRq.getTxnAdditionalDtls().getConsolidatedChargeAmount().getIsoCurrencyCode()
							? remittanceRq.getTxnAdditionalDtls().getConsolidatedChargeAmount().getIsoCurrencyCode()
							: StringUtils.EMPTY);
			preCalculatedCharges.setSuppressAdditionalCharges(Boolean.TRUE);
			initiationContext.setPreCalculatedCharges(preCalculatedCharges);
		}
	...
	private PaymentInstruction16 preparePaymentInstruction(TellerRemittanceRq remittanceRq) {
		...
		pmtInstruction
				.setPmtInfId(RemittanceHelper.checkNullValue(remittanceRq.getTxnAdditionalDtls().getRemittanceId()));
		...
		if (remittanceRq.getInitiateSwiftMessageRqDtls().getTransactionDetails().getCreditPostingDtls()
				.getCreditExchangeRate().compareTo(BigDecimal.ONE) > 0) {
			// exchange Rate info
		...
		if (StringUtils.isNotBlank(remittanceRq.getInitiateSwiftMessageRqDtls().getBeneficiaryCustomerAndInstitution()
			...
			// nostro accountId
			cdtrAcctOthr.setId(remittanceRq.getInitiateSwiftMessageRqDtls().getTransactionDetails()
		...
		if (null != remittanceRq.getInitiateSwiftMessageRqDtls().getIntermediaryDetails().getIntermediary()) {
		...
		splmtryDataList.add(prepareSupplementaryData(remittanceRq));
		...
		if (null != remittanceRq.getInitiateSwiftMessageRqDtls().getDocumentUpload()) {
			...
			if (vDocumentUploadArray.length > 0) {
				...
				for (DocumentUploadDtls doc : vDocumentUploadArray) {
					...
					docRefNumber.setNb(doc.getDocumentSavedId());
	 ...
	 * @return
	 */
	private SupplementaryData1 prepareSupplementaryData(TellerRemittanceRq remittanceRq) {
		...
		if (remittanceRq.getTxnAdditionalDtls().getFundingMode().equals("CASH")) {
			document.setCashIndication("CASH");
		}
		document.setBranchId(BankFusionThreadLocal.getUserSession().getBranchSortCode());
		document.setTransferMethod(remittanceRq.getInitiateSwiftMessageRqDtls().getMessageDetails().getMessagePreference().equals("SERIAL") ? "S" : "C");		
