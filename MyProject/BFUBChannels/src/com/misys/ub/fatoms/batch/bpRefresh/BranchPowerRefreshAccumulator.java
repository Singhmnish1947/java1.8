...
/**
 * ForexDealsPnLAccumulator is able to be merged into one class in order to derive amounts processed by all process classes on
 * their own threads.
 *
 *
 * @see com.trapedza.bankfusion.batch.process.AbstractProcessAccumulator
 */
public class BranchPowerRefreshAccumulator extends AbstractProcessAccumulator {
	/**
	 ...
	 *            <code>AbstractProcessAccumulator</code> to set arguments. In addition this extention sets up BigDecimal arrays
	 *            for debit and credit amounts with zero.
	 */
	public BranchPowerRefreshAccumulator(Object[] args) {
		super(args);
	}
	...
	/**
	 * Supporting a form of the Builder Pattern, this method adds other accumulators from the batch run to this one so that merged
	 * totals can be gathered.
	 *
	 * @param accumulator
	 *            The accumulator
	...
	/**
	 * This method processes totals from all accumulators in the batch run. See above method addAccumulatorForMerging() for where
	 * it gets the accumulators from.
	 *
