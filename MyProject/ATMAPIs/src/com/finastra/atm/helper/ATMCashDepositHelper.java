...
public class ATMCashDepositHelper {
	...
	public void updateATMActivity(CashDepositRequest cashDeposit, String errorCode, String errorCodeMsg) {

		factory = BankFusionThreadLocal.getPersistanceFactory();
		String msgFunction = String.valueOf(cashDeposit.getMessageFunction());
		int forcePost = 0;
		...
		String atmActivityIdPk = GUIDGen.getNewGUID();
		iboATMActivityDetail.setBoID(atmActivityIdPk);
		iboATMActivityDetail.setF_SOURCECIB(cashDeposit.getAcquiringInstitutionIdentificationCode());
		iboATMActivityDetail.setF_ACCOUNTID(cashDeposit.getAccountIdentification2());

		iboATMActivityDetail.setF_ACCOUNTCURRENCY(cashDeposit.getAmountCurrencyCardholderBilling());
		iboATMActivityDetail.setF_MSGRECVDATETIME(cashDeposit.getTimeLocalTransaction());
		if (CommonUtil.checkIfNotNullOrEmpty(cashDeposit.getPrimaryAccountNumberIdentifier())) {
			iboATMActivityDetail.setF_ATMCARDNUMBER(cashDeposit.getPrimaryAccountNumberIdentifier());
		} else {
			iboATMActivityDetail.setF_ATMCARDNUMBER("API");
		}
		iboATMActivityDetail.setF_ATMDEVICEID(cashDeposit.getCardAcceptorTerminalIdentification());
		...
		String transactionCode = getTransactionCode((ATMEssenceRequestConstant.ATM_CASH_WITHDRAWAL),
				cashDeposit.getMessageFunction(), cashDeposit.getChannelId());
		...
		iboATMActivityDetail.setF_POSTDATETIME(cashDeposit.getDateTimeTransmission());
		iboATMActivityDetail.setF_TRANSACTIONDTTM(cashDeposit.getDateTimeTransmission());
		iboATMActivityDetail.setF_TRANSACTIONREFERENCE(cashDeposit.getRetrievalReferenceNumber());

		factory.create(IBOATMActivityDetail.BONAME, iboATMActivityDetail);
		updateATMActivityISOExtn(cashDeposit, atmActivityIdPk);
		factory.commitTransaction();
	}

	private void updateATMActivityISOExtn(CashDepositRequest cashDeposit, String atmActivityIdPk) {
		...
		iboISOATM_ActivityUpdate.setF_UBACTUALTXNAMOUNT(cashDeposit.getAmountTransaction());
		iboISOATM_ActivityUpdate.setF_UBACTUALTXNFEE(cashDeposit.getAmountSettlement());
		iboISOATM_ActivityUpdate.setF_UBAQUIRERID(cashDeposit.getAcquiringInstitutionIdentificationCode());
		iboISOATM_ActivityUpdate.setF_UBCARDACCEPTORDATA(cashDeposit.getCardAcceptorNameLocation());
		iboISOATM_ActivityUpdate.setF_UBCARDACCEPTORID(cashDeposit.getCardAcceptorIdentificationCode());
		iboISOATM_ActivityUpdate.setF_UBCARDISSUERDATA(cashDeposit.getCardIssuerReferenceNumber());
		iboISOATM_ActivityUpdate.setF_UBPROCESSINGCODE(ATMAPIConstant.processingCodeTransactionTypeCdp);
		iboISOATM_ActivityUpdate.setF_UBRECEIVINGINSTID(cashDeposit.getReceivingInstitutionIdentificationCode());
		factory.create(IBOISOATM_ActivityUpdate.BONAME, iboISOATM_ActivityUpdate);
	}
